...
export default function BillDetail() {
  ...
  const [voucherForm, setVoucherForm] = useState({ amount: '', narration: '', supplier_bank_id: '', due_days: '' });
  ...
  async function fetchBill() {
    try {
      ...
      setForm({
        ...
        invoice_date:       data.bill.invoice_date ? data.bill.invoice_date.slice(0, 10) : '',
        bill_ref_no:        data.bill.bill_ref_no || '',
        bill_date:          data.bill.bill_date ? data.bill.bill_date.slice(0, 10) : '',
  ...
  async function fetchMeta() {
    const [sRes, bRes] = await Promise.all([
      fetch('/api/suppliers?approved_only=true', { headers: authH }),
      fetch('/api/company', { headers: authH }),
    ]);
    const [sData, bData] = await Promise.all([sRes.json(), bRes.json()]);
    setSuppliers(sData.suppliers || []);
    const allBanks = [];
    for (const co of (bData.companies || [])) {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          due_days:       Number(form.due_days)       || null,
        }),
      });
      const data = await res.json();
    ...
    try {
      const res = await fetch(`/api/bills/${id}/voucher`, {
        ...
        body: JSON.stringify({
          ...
          due_days:        voucherForm.due_days ? Number(voucherForm.due_days) : null,
        }),
      });
      const data = await res.json();
      ...
      setVoucherForm({ amount: '', narration: '', supplier_bank_id: '', due_days: '' });
      ...
      <div style={{ padding: '24px 32px', maxWidth: 960, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {!isNew && !editMode && ['admin', 'executive'].includes(user?.role) && (
              <>
                ...
                <button
                  onClick={() => {
                    setVoucherForm(f => ({ ...f, amount: remaining > 0 ? remaining.toFixed(2) : '', due_days: bill?.due_days ?? '', supplier_bank_id: supplierBanks.find(b => b.is_primary)?.id || supplierBanks[0]?.id || '' }));
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e9ecef', padding: 20 }}>
            ...
            {editMode ? (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[['taxable_amount','Taxable Amount'],['cgst','CGST'],['sgst','SGST'],['igst','IGST'],['tds_amount','TDS']].map(([k, label]) => (
                  <div key={k}>
              ...
              <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#f8f9fa', borderBottom: '2px solid #dee2e6' }}>
                    {['Voucher No', 'Amount', 'Status', 'Payment', 'UTR No', 'Created'].map(h => (
